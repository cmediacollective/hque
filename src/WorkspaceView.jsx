                    ...
                    <div style={{ flex: 1, overflowY: 'auto', padding: '10px 10px 0' }}>
                      {sortTasks(tasks.filter(t => t.column_id === col.id)).map(task => (
                        <div key={task.id}
                          ...
                          style={{ background: card, border: `0.5px solid ${border}`, borderRadius: '6px', padding: '16px', marginBottom: '8px', cursor: 'pointer', opacity: doneColumnIds.has(task.column_id) ? 0.55 : 1, boxShadow: hoveringTask === task.id ? taskShadowHover : taskShadow, transform: hoveringTask === task.id ? 'translateY(-2px)' : 'none', transition: 'box-shadow 0.15s ease, transform 0.15s ease', fontFamily: "'Inter Tight', 'Helvetica Neue', Helvetica, Arial, sans-serif" }}>
                          <div style={{ fontSize: '13px', fontWeight: 600, color: text, lineHeight: 1.4, marginBottom: '8px', textDecoration: doneColumnIds.has(task.column_id) ? 'line-through' : 'none' }}>{task.title}</div>
                  ...
                  <div key={col.id} style={{ marginBottom: '24px' }}>
                    ...
                    {tasks.filter(t => t.column_id === col.id).map(task => (
                      <div key={task.id}
                        ...
                        onMouseEnter={() => setHoveringTask(task.id)}
                        onMouseLeave={() => setHoveringTask(null)}
                        style={{ background: card, border: `0.5px solid ${border}`, borderRadius: '6px', padding: '16px 18px', marginBottom: '8px', cursor: 'pointer', display: 'grid', gridTemplateColumns: '1fr 80px 90px auto 24px', gap: '12px', alignItems: 'center', opacity: doneColumnIds.has(task.column_id) ? 0.55 : 1, boxShadow: hoveringTask === task.id ? taskShadowHover : taskShadow, transform: hoveringTask === task.id ? 'translateY(-2px)' : 'none', transition: 'box-shadow 0.15s ease, transform 0.15s ease', fontFamily: "'Inter Tight', 'Helvetica Neue', Helvetica, Arial, sans-serif" }}>
                        <div>
                          <div style={{ fontSize: '13px', fontWeight: 600, color: text, marginBottom: '3px', textDecoration: doneColumnIds.has(task.column_id) ? 'line-through' : 'none' }}>{task.title}</div>
