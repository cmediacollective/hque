  ...
  return (
    <div style={{ display: 'flex', flex: 1, overflow: 'hidden', background: bg }}>
          ...
          <>
            <div style={{ padding: '18px 28px', display: 'flex', alignItems: 'center', gap: '14px', borderBottom: `0.5px solid ${border}`, flexShrink: 0 }}>
              ...
              {selectedBrand.id === '__internal' ? (
                ...
                <img src={selectedBrand.logo_url} alt={selectedBrand.name} style={{ width: '44px', height: '44px', objectFit: 'contain', borderRadius: '4px', background: '#fff', padding: '3px', border: `0.5px solid ${border}`, flexShrink: 0 }} onError={e => { e.target.style.display = 'none' }} />
              ) : (
                <div style={{ width: '44px', height: '44px', borderRadius: '4px', background: colorFromName(selectedBrand.name), color: '#fff', fontSize: '15px', fontWeight: 500, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{(selectedBrand.name || '?').charAt(0).toUpperCase()}</div>
              )}
              <div style={{ flex: 1 }}>
                    ...
                    <div style={{ flex: 1, overflowY: 'auto', padding: '10px 10px 0' }}>
                      {sortTasks(tasks.filter(t => t.column_id === col.id)).map(task => (
                        <div key={task.id}
                          ...
                          style={{ background: card, border: `0.5px solid ${border}`, borderRadius: '6px', padding: '16px', marginBottom: '8px', cursor: 'pointer', opacity: doneColumnIds.has(task.column_id) ? 0.55 : 1, boxShadow: hoveringTask === task.id ? taskShadowHover : taskShadow, transform: hoveringTask === task.id ? 'translateY(-2px)' : 'none', transition: 'box-shadow 0.15s ease, transform 0.15s ease', fontFamily: "'Plus Jakarta Sans', 'Helvetica Neue', Helvetica, Arial, sans-serif" }}>
                          <div style={{ fontSize: '14px', fontWeight: 600, color: text, lineHeight: 1.4, marginBottom: '8px', textDecoration: doneColumnIds.has(task.column_id) ? 'line-through' : 'none' }}>{task.title}</div>
                          {task.description && <div style={{ fontSize: "12px", fontWeight: 400, color: muted, opacity: 0.6, lineHeight: 1.5, marginBottom: "8px", whiteSpace: "pre-wrap", display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{task.description}</div>}
                  ...
                  <div key={col.id} style={{ marginBottom: '24px' }}>
                    ...
                    {tasks.filter(t => t.column_id === col.id).map(task => (
                      <div key={task.id}
                        ...
                        onMouseEnter={() => setHoveringTask(task.id)}
                        onMouseLeave={() => setHoveringTask(null)}
                        style={{ background: card, border: `0.5px solid ${border}`, borderRadius: '6px', padding: '16px 18px', marginBottom: '8px', cursor: 'pointer', display: 'grid', gridTemplateColumns: '1fr 80px 90px auto 24px', gap: '12px', alignItems: 'center', opacity: doneColumnIds.has(task.column_id) ? 0.55 : 1, boxShadow: hoveringTask === task.id ? taskShadowHover : taskShadow, transform: hoveringTask === task.id ? 'translateY(-2px)' : 'none', transition: 'box-shadow 0.15s ease, transform 0.15s ease', fontFamily: "'Plus Jakarta Sans', 'Helvetica Neue', Helvetica, Arial, sans-serif" }}>
                        <div>
                          <div style={{ fontSize: '14px', fontWeight: 600, color: text, marginBottom: '3px', textDecoration: doneColumnIds.has(task.column_id) ? 'line-through' : 'none' }}>{task.title}</div>
      ...
      {showNotes && selectedBrand && selectedBrand.id !== '__internal' && (
        <BrandNotes brand={selectedBrand} userId={userId} agencyTz={agencyTz} dark={dark} orgId={orgId} members={members} onClose={() => setShowNotes(false)} />
      )}
    </div>
  )
}
