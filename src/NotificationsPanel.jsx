        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          {loading ? (
            ...
            <div style={{ padding: '60px 20px', textAlign: 'center' }}>
              <svg width='28' height='28' viewBox='0 0 24 24' fill='none' stroke='currentColor' strokeWidth='1.5' strokeLinecap='round' strokeLinejoin='round' style={{ color: subtle, marginBottom: '12px' }}><path d='M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9'/><path d='M13.73 21a2 2 0 0 1-3.46 0'/></svg>
            ...
            <div key={n.id} onClick={() => {
              markRead(n.id)
              if (n.task_id && onOpenTask) { onOpenTask(n.task_id); onClose && onClose() }
              else if (n.campaign_id && onOpenCampaign) { onOpenCampaign(n.campaign_id); onClose && onClose() }
            }} style={{ padding: '14px 20px', borderBottom: `0.5px solid ${border}`, cursor: 'pointer', background: n.read ? 'transparent' : (dark ? 'rgba(91,124,153,0.06)' : 'rgba(91,124,153,0.04)'), display: 'flex', gap: '12px', alignItems: 'flex-start' }}
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: '10px', color: subtle, display: 'flex', gap: '8px', alignItems: 'center' }}>
                  ...
                  {n.task_id
                    ? <span style={{ color: '#5b7c99', letterSpacing: '0.14em', textTransform: 'uppercase', fontSize: '9px' }}>Open task →</span>
                    : n.campaign_id
                    ? <span style={{ color: '#5b7c99', letterSpacing: '0.14em', textTransform: 'uppercase', fontSize: '9px' }}>Open campaign →</span>
                    : null}
                </div>
              </div>
            </div>
