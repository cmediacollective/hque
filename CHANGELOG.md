...
**Phase 3 marketing landing rewrite (preview branch only).** The homepage `/` is fully rewritten as an editorial design-studio layout, consuming the phase-1 design tokens and the phase-2 components. Shipped on a `marketing-redesign` branch — Netlify will produce a deploy preview URL; main is untouched. Page structure: minimal nav (wordmark + mono links + underlined "sign in", no buttons); asymmetric hero (60/40 split, giant italic Fraunces headline left, 60-word description plus "(scroll) ↓" bottom-right, full viewport height); section 01 hover-reveal list of the five things HQue does (roster, campaigns, workspace, reports, inquiries — each with caption and image, accent-blue section label); section 02 full-bleed editorial image with mono caption ("inside a talent agency on a tuesday morning. los angeles, march."); section 03 pricing as a typographic table — three rows, plan name italic Fraunces 56px left, price Fraunces 56px right, mono description below, 4rem gaps, no borders/cards/checkmarks; section 04 a second full-bleed editorial image; footer with three mono columns (contact, credits, legal) plus a giant Fraunces "HQue" wordmark filling the page width and bleeding off slightly. Voice rules followed: declarative, sentence fragments, italics only for emphasis, zero em dashes anywhere on the page. Accent blue used in exactly two moments: link hover, and the "01 / what it does" section label. Placeholder editorial images live at `/public/marketing/{editorial-01.jpg, editorial-02.jpg, reveals/01-05.jpg}` — drop replacement files at the same paths to swap them, no code change needed. Other marketing pages (/pricing, /faq, /blog) still render the old design and will be addressed in a later phase.
